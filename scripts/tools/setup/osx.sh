...
###############################################################################
# diskspaced / System Integrity Protection                                    #
###############################################################################

# Stop the service
sudo launchctl unload -w /System/Library/LaunchAgents/com.apple.diskspaced.plist &>/dev/null
sudo launchctl stop com.apple.diskspaced &>/dev/null
sudo killall -STOP diskspaced

# Set a better limit
defaults write com.apple.diskspaced freeSpaceWarningLevel 1
defaults write com.apple.diskspaced warningInterval 3600
defaults write com.apple.diskspaced minFreeSpace 3 # 3GB ?
defaults write com.apple.diskspaced debugLog 1
defaults write com.apple.diskspaced checkAllVolumes 0

# Try to start the service again
sudo launchctl load -w /System/Library/LaunchAgents/com.apple.diskspaced.plist &>/dev/null
sudo launchctl start com.apple.diskspaced &>/dev/null
